...
def list_environments(project_name):
...
@click.option("--size", "-s", default="medium", help="Size profile: small, medium, or large")
@click.option("--branch", "-b", default=None, help="Git branch name")
@click.option("--domain", "-d", default=None, help="Domain for this environment")
@click.option("--deploy", "do_deploy", is_flag=True, help="Deploy infrastructure immediately")
def add_environment(project_name, environment, size, branch, domain, do_deploy):
    """
...
def remove_environment(project_name, environment, force):
...
@env_group.command(name="info")
@click.argument("project_name")
@click.argument("environment")
def environment_info(project_name, environment):
