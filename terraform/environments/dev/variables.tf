...
variable "media_bucket_size_gb" {
  description = "Maximum size for media bucket in GB (Civo minimum: 500GB)"
  type        = number
  default     = 500
}

variable "static_bucket_size_gb" {
  description = "Maximum size for static bucket in GB (Civo minimum: 500GB)"
  type        = number
  default     = 500
}
...
# Load Balancer Variables (optional)
variable "app_hostname" {
  description = "Hostname for the application load balancer"
  type        = string
  default     = ""
}
